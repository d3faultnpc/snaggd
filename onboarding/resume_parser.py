...
class ResumeParser:
    ...
    def _extraction_prompt(self) -> str:
        return (
            ...
            "C3 — A venture with an organisation, a role AND a period of its own is "
            "type='employment', however the CV files it. Someone's own company, a startup they "
            "founded, a studio they run — a section headed 'Projects' or 'Side projects' does not "
            "make it one; what makes an entry a project is the ABSENCE of that triple. So "
            "'snaggd | Founder | 2026–present' is employment, and 'AI Health Assistant', named "
            "inside a job with no organisation and no dates of its own, is not an entry at all — "
            "it is a line of the entry it sits under (see A). This rule is about employment vs "
            "project ONLY: education and credential keep their own types, triple or not.\n"            "C2 — Volunteering: unpaid work with an organisation or cause → "
