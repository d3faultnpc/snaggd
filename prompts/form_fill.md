...
- The field's type is what the form will accept; the question's wording is not. A radio_group or select takes exactly ONE option even where the question invites several; a checkbox_group takes several even where the question reads as one choice. When the wording and the type disagree, the type wins: for a single-choice field pick the one option that fits the candidate best, and never list more than one
